...
import { addBestDoctorHyperlocal, getHyperlocalcategoryList } from '@/ApiActions/Admin/hyperlocaladmin/hyperlocal';
import { doctorsListFrontend,  fetchAllSpecialty } from "@/ApiActions/CommonApi";


const AddBestTreatment = () => {
    ...
    const [userPhotoaboutus, setUserPhotoaboutus] = useState(null);
    ...
    const [relatedDoctorOptions, setRelatedDoctorOptions] = useState([]);
        ...
        const fetchDoctors = async () => {
            try {
                ...
                const options = doctors.map((doctor) => ({
                    ...
                    image: doctor.featured_images
                        ? `${process.env.REACT_APP_FILE_BASE_URL}/${doctor.featured_images}`
                        : `${process.env.REACT_APP_FILE_BASE_URL}/images/placeholder-image.webp`,
                }));
                setRelatedDoctorOptions(options);
            } catch (error) {
                console.error('Error fetching doctor list:', error);
            }
        };

        fetchDoctors(); // Call the async function inside useEffect
    }, []);
    ...
    const showImagePreviewAndValidateaboutus = (e) => {
        try {
            let selectedFileaboutus = e.target.files.item(0);
            let supportedFormats = ['image/jpeg', 'image/png', 'image/webp'];
            if (!selectedFileaboutus) throw new Error('No file selected.');
            if (!supportedFormats.includes(selectedFileaboutus.type)) throw new Error('Invalid file type. Only JPEG/PNG/webp are allowed.');
            if (selectedFileaboutus.size > 5 * 1024 * 1024) throw new Error('File size is more than 5 MB');

            let fileReader = new FileReader();
            fileReader.onload = (event) => {
                setUserPhotoaboutus(event.target.result);
            };
            fileReader.readAsDataURL(selectedFileaboutus);
            setSelectedFileAboutUs(selectedFileaboutus);
        } catch (error) {
            console.error(error);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (selectedFileaboutus) {
            data.append('aboutus_images', selectedFileaboutus);
        }
        ...
        data.append('related_doctor', formData.relateddoctor.map(option => option.value).join(','));
        ...
        try {
            const response = await addBestDoctorHyperlocal(data);
                                                ...
                                                <div id="comprehensivecare" className="tab-pane fade">
                                                    ...
                                                    {formData.comprehensivecare?.map((cares, index) => (
                                                        <div key={index} className="border rounded p-3 mb-4 position-relative">
                                                            ...
                                                            {/* Image Preview */}
                                                            {/* {console.log(comprehensiveImages[index])} */}
                        ...
                        <div className="col-md-3">
                            <div className="status-sec">
                                <div className="input-outer">
                                    ...
                                    <div className="mt-3">
                                        <FileControl label={'Abouts Us Image'} onChange={showImagePreviewAndValidateaboutus} />
                                        {userPhotoaboutus ? (
                                            <img className="image_preview" src={userPhotoaboutus} alt="User Photo" />
                                        ) : null}
                                    </div>
...
export default AddBestTreatment;
